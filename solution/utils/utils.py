...
class ZKP_IdP(ZKP, Cipher_Authentication):
	def __init__(self, methods: str, minimum_methods: int, key: bytes, max_iterations: int):
		ZKP.__init__(self, password=b'')
		Cipher_Authentication.__init__(self, key=key)

		self.methods: list[str] = json.loads(base64.urlsafe_b64decode(methods).decode())
		self.minimum_methods = minimum_methods

		self.current_method = ''
		self.methods_successful = set()
		self.methods_unsuccessful = set()
